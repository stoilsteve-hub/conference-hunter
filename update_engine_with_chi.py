...
for line in text.strip().split("\n"):
    ...
    if len(parts) >= 3:
        ...
        if domain not in existing_map:
            existing_map[domain] = (spider_cls, name, "Both", url)
...
for d, v in existing_map.items():
    s_class = "ImmunoOncologySpider" if v[0] == "ImmunoOncologySpider" else v[0]
    if isinstance(s_class, type): s_class = s_class.__name__
    name = v[1]
    topic = v[2]
    s_url = v[3]
